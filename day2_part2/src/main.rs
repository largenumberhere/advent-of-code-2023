...
fn main() {
    ...
    for (game_number, game) in games {
        // calculate min required cubes per game
        let (reds, greens, blues) = min_required_by_color(&game);

        let cubes_power = {
            let mut cubes_power = reds;
            cubes_power = cubes_power.checked_mul(greens).unwrap();
            cubes_power = cubes_power.checked_mul(blues).unwrap();

            cubes_power
        };

        println!("Game {}: requires {} red cubes, {} green cubes, {} blue cubes. Cube power: {}", game_number, reds, greens, blues, cubes_power);
        total_power = total_power.checked_add(cubes_power).unwrap();
    }

    println!("Games total power: {}", total_power);
}


fn min_required_by_color(game: &Game) -> (u32, u32, u32) {

    // get most used of each cube per game
    let mut reds = 0;
    let mut greens = 0;
    let mut blues = 0;
