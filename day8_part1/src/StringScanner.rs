...
use std::ops::BitAnd;
use std::path::Path;
use std::str::{from_boxed_utf8_unchecked, from_utf8, FromStr};
...
impl FileParser<BufReader<File>> {
    pub fn new_from_path(path: impl AsRef<Path>) -> Result<FileParser<BufReader<File>>, std::io::Error> {
...
impl<R: Read> FileParser<R>{
    ...
    /// returns true if newline or eof is next
...
TODO: make this into tests
    ...
    let mut scanner = StringScanner::FileParser::new_from_reader(file);
    // for i in 0 ..50 {
